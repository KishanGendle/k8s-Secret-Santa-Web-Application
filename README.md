...
10. [AWS EC2 Setup](#aws-ec2-setup)
11. [Docker Setup](#docker-setup)
12. [Kubernetes Cluster Creation Using kubeadm](#kubernetes-cluster-creation-using-kubeadm)
...
This project was deployed on AWS EC2 instances using a Kubernetes cluster created with kubeadm.
...
```bash
docker tag secret-santa-app:latest <dockerhub-username>/secret-santa-app:latest

docker push <dockerhub-username>/secret-santa-app:latest
```
...
This cluster uses **containerd** as the Container Runtime Interface (CRI).

### Install Kubernetes Components

Install the following on Master and Worker nodes:
...
Calico CNI is used to provide networking between Master and Worker nodes and enable Pod-to-Pod communication.
...
Run the command generated by `kubeadm init` on each worker node.
...
```bash
kubectl get all -n secret-santa
```

### Verify Pods

```bash
kubectl get pods -n secret-santa
```

### Verify Services

```bash
kubectl get svc -n secret-santa
```

### Port Forward Application

```bash
kubectl port-forward svc/flask-service 8000:80 -n secret-santa
```
